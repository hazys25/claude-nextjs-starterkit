...
- `src/lib/api/api-response.ts` — `successResponse(data, status?)` / `errorResponse(code, message, status?, context?)`. **`errorResponse` 는 내부에서 `logger.error` 로 자동 로그 적재**하므로, 에러 응답을 만들면 로깅 규칙이 함께 충족된다.
- `src/lib/api/logger.ts` — 공통 로거(현재 콘솔 출력, 인터페이스만 쓰면 추후 외부 서비스로 교체 용이).
